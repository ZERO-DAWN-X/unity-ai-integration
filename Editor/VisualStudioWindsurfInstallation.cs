...
";
			...
			try {
				...
				foreach (var process in processes) {
					try {
						...
						if (commandLine.Contains(solutionPath))
							return process;
					} catch (Exception) {
		...
		public override bool Open(string path, int line, int column, string solution) {
			...
			// Try to find an existing Windsurf instance with the solution open
			var existingProcess = FindRunningWindsurfWithSolution(solution);
			if (existingProcess != null) {
				// If Windsurf is already running with this solution, just open the file
